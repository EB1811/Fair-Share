...
const RemoteGoodsQuestionnairePage = (props) => {
    useFirestoreConnect([
        ...
        if (!auth.isEmpty) {
            if (session && session.active && session.owner === auth.uid) {
                if (stage === "0") {
                            ...
                            <Row className='justify-content-center align-items-center min-vh-100'>
                                ...
                                >
                                    ...
                                    <Button
                                        ...
                                        onClick={() =>
                                            props.history.push(
                                                `/Distribute/Questions/Remote/Goods/${sessionID}/1`
                                            )
                                        }
                                    >
                    ...
                    return (
                        <Redirect
                            to={`/Distribute/GoodInfo/Remote/Goods/${sessionID}`}
                        />
                    );
                } else {
                    return (
                        <Redirect
                            to={`/Distribute/Questions/Remote/Goods/${sessionID}/0`}
                        />
                    );
                }
            } else {
...
export default RemoteGoodsQuestionnairePage;
